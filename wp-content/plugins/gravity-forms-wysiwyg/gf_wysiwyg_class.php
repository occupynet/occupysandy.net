...
if (!class_exists('gf_wysiwyg_logic')) {
	...
	class gf_wysiwyg_logic {
		...
		function add_gf_tooltips($tooltips){
			$tooltips["form_field_enable_wysiwyg"] = "<h6>Enable WYSIWYG</h6>Check this box to turn this field into a WYSIWYG editor.";
			return $tooltips;
		}
		...
		function is_wysiwyg($field) {
			...
			if ($type == 'wysiwyg') {
				return true;
			} else if ($type = 'post_content' || $type = 'textarea') {
		...
		function render_editor_js() {
			...
			<script type='text/javascript'>
				...
				jQuery(document).bind("gform_load_field_settings", function (event, field, form) {
					...
					if (field_type == 'post_content' || field_type == 'textarea' || (field_type == 'post_custom_field' && field['inputType'] == 'textarea')) {
