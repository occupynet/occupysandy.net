...
HTML;
	...
	private function render_category_tag_div(
		...
		$html .= '<span class="ai1ec-facebook-comments">' .
			__( 'Allow comments', AI1EC_PLUGIN_NAME )
			. ': <strong>';
		$html .= ( $comments_enabled )
			? __( 'Yes', AI1EC_PLUGIN_NAME )
			: __( 'No',  AI1EC_PLUGIN_NAME );
		$html .= '</strong></span>';
		$html .= '<span class="ai1ec-facebook-map">' .
			__( 'Show map', AI1EC_PLUGIN_NAME )
			. ': <strong>';
...
HTML;
	...
	public function create_select_category( $id ) {
		$select = '<select class="ai1ec-categories-selector span12" name="' . $id . '" id="' . $id . '">';
		// Set an empty option
	...
	private function render_multiselects( array $types, $current_id ) {
		...
		$category_select = $this->create_select_category( 'ai1ec_facebook_feed_category' );
...
<div class="row-fluid ai1ec_submit_wrapper">
	...
	<div class="ai1ec-feed-tags span6">
		...
		<input type="text" class="span12" name="ai1ec_facebook_feed_tags"
